...
alter table public.students
  ...
  add column if not exists has_tok boolean,
  add column if not exists tok_course text;

alter table public.student_enrollments
  add column if not exists block_code text
    check (block_code is null or block_code in ('A', 'B', 'C', 'D', 'E', 'F'));

create or replace function public.block_code_for_course_name(course_name text)
returns text
language sql
immutable
as $$
  select upper((regexp_match(coalesce(course_name, ''), '^Block\s+([A-F])'))[1]);
$$;
...
create or replace function public.sync_student_slot_assignments_from_enrollment_change()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  perform public.sync_student_slot_assignments_for_student(coalesce(new.student_id, old.student_id));
  return coalesce(new, old);
end;
$$;

create or replace function public.sync_student_slot_assignments_from_student_change()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  perform public.sync_student_slot_assignments_for_student(new.student_id);
  return new;
end;
$$;
...
after insert or update of program, has_tok, tok_course on public.students
for each row
