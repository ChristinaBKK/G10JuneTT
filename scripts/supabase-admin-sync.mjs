...
async function upsertStudents(filePath) {
  ...
  payload.forEach((student) => {
    const uniqueCourses = [...new Set(student.enrollments || [])];
  ...
  await request(`/rest/v1/student_enrollments?student_id=in.(${studentIds.join(',')})`, {
    method: 'DELETE',
    headers: {
  ...
  if (enrollmentRows.length > 0) {
    await request('/rest/v1/student_enrollments', {
      ...
      body: JSON.stringify(enrollmentRows),
    });
  }
